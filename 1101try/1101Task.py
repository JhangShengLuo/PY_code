...
def insert_data():
    ...
    cursor = cnx.cursor()
    files = []
    for f in os.listdir('/Users/Jackie/Desktop/1101try/done/'):
        if os.path.isfile('/Users/Jackie/Desktop/1101try/done/' + f):
            files.append(f)
    replace2 = []
    for f in files:
        if '.csv' in f:
            ...
            # 讀取要寫入的檔案
            with io.open('/Users/Jackie/Desktop/1101try/done/' + f, 'r', encoding='utf-8') as rcsv:
                content = []
                for line in rcsv:
                    content.append(line)
...
#             library(grid)
#             library(formattable)
#             setwd("/Users/Jackie/Desktop/1101try/")

#             Sys.setlocale(category = "LC_CTYPE", locale= "zh_CN.UTF-8")

#             m1=read.table("/Users/Jackie/Desktop/1101try/allappend.csv",  header = TRUE, sep = ",")
